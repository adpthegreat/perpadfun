        ...
        try {
          ...
          if (verifiedAttached) {
            ...
            }
            {
              ...
              // Hard fallback: NEVER leave launch_mid null at open. Imperial often
              // returns no entry price; if resolveImperialEntryPrice's own mark
              // fallback also missed, capture the live mark now. Otherwise the
              // entry-capture window (ENTRY_CAPTURE_WINDOW_MS) closes and the keeper's
              // PnL is stuck on Imperial's blended ~0 for the life of the position
              // (which disables take-profit). See KEEPER_PNL.md.
              let entryPrice = entry?.price ?? null;
              if (!entryPrice) {
                entryPrice = await readImperialLiveMarkUsd(ctx.underlying).catch(() => null);
              }
