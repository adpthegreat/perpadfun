...
const spec = {
  ...
  paths: {
    "/api/v1/markets": {
      get: {
        summary: "List launchable markets + leverage caps",
    ...
    "/api/v1/launch": {
      post: {
        ...
        description:
          "Returns unsigned config + pool txs; the caller signs both as payer/buyer and the pool tx carries the 0.01 SOL fee. A row is recorded and promoted to `live` when the pool confirms on-chain.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
              schema: {
                ...
                properties: {
                  ...
                  creatorAddress: {
                    type: "string",
                    description: "your wallet — signer, payer, buyer, dev-buy recipient",
                  },
                  devBuy: {
                    type: "number",
                    description: "dev-buy in quote UI units (SOL 0.1–5, USDC 5–5000)",
                  },
        ...
        responses: {
          "200": {
            description:
              "{ tokenId, mint, configAddress, poolAddress, transactions[], supplyBreakdown, protocolFeeSol }",
          },
    ...
    "/api/v1/launch/{tokenId}/metadata": {
      get: {
        ...
        parameters: [{ name: "tokenId", in: "path", required: true, schema: { type: "string" } }],
        responses: {
          "200": { description: "Metaplex metadata" },
          "404": { description: "not yet confirmed" },
        },
      },
    },
  },
