...
import { clientIp, ipHash, randomReferralCode } from "@/lib/quest/server";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Resolve the referrer only if well-formed and real; never let a wallet refer itself
        // (impossible here since this row does not exist yet).
